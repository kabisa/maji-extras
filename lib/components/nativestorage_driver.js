...
(function() {
  ...
  _support = function() {
    return typeof window.NativeStorage !== 'undefined';
  };
  ...
  _initStorage = function(options) {
    ...
    return localforage.getSerializer().then(function(serializer) {
      ...
      return _getKeys(dbInfo)["catch"](function(error) {
  ...
  _clear = function(keys, prefix) {
    ...
    if (keys.length > 0) {
      ...
      NativeStorage.remove(prefix + key, function() {
        return _clear(keys.slice(1), prefix).then(deferred.resolve)["catch"](deferred.reject);
      }, deferred.reject);
    } else {
      deferred.resolve();
    }
    ...
    if (keys.length > 0) {
      ...
      NativeStorage.getItem(dbInfo.dataKeyPrefix + key, function(serializedValue) {
        ...
        if (iterationResult !== void 0) {
          ...
          return _iterate(keys.slice(1), dbInfo, iterator, index + 1).then(deferred.resolve)["catch"](deferred.reject);
        }
      }, deferred.reject);
    } else {
      deferred.resolve();
  ...
  removeItem = function(key, callback) {
    ...
    self.ready().then(function() {
      ...
      return NativeStorage.remove(dbInfo.dataKeyPrefix + key, function() {
        return _removeKey(key, dbInfo).then(function() {
